...
# Load YOLO models
dog_detector = YOLO("models/yolov8m.pt")
behavior_model = YOLO("models/best.pt")
...
def detect_emotion_from_image(image_path):
    try:
        ...
        # Always save a preview image for the frontend
        preview_filename = f"preview_{uuid.uuid4().hex}.jpg"
        preview_path = os.path.join(TEMP_DIR, preview_filename)
        preview_bgr = cv2.cvtColor(roi, cv2.COLOR_RGB2BGR)
        cv2.imwrite(preview_path, preview_bgr)
        image_url = f"/static/{preview_filename}"
